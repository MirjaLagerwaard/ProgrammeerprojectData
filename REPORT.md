...
**Interaction:** the color of the circles on the map changes on-hover. When you click on a circle you will see a tooltip with details about that particular incident. I have chosen for the on-click interactivity, because the circles are very close to each other on the map. When you change this to an on-hover interactivity it is harder to see the details of one particular incident. The cursor will change to an pointer to make clear that it is possible to click on a circle. There are also zoombuttons to make it easier for the user to zoom into the cities where most of the incidents took place.
